...
void SchedInit(int size, schedcfg_t *cfg)
{
  ...
  if (cfg != NULL) {
    ...
    config.node = -1;
    config.do_print_schedinfo = false;
  ...
  for( i=0; i<num_workers; i++) {
    ...
    sc->mon = MonitoringCreate( config.node, wname);
...
void SchedWrapper(task_t *t, char *name)
{
  ...
  sc->mon = MonitoringCreate( config.node, name);
...
static void Worker( schedctx_t *sc)
{
  ...
  do {
    ...
    while( 0 == sc->queue.count &&
      ...
      /* TODO only on flag */
      MonitoringDebug( sc->mon, "wid %d waiting (queue: %u, tasks: %u, term: %d)\n",
...
#ifdef MONITORING_ENABLE
  MonitoringDebug( sc->mon, "worker %d exited (queue: %u, tasks: %u, term: %d)\n",
      sc->wid, sc->queue.count, sc->num_tasks, sc->terminate);
#endif
}
...
static void Wrapper( schedctx_t *sc)
{
  ...
  assert( sc->num_tasks == 0);
#ifdef MONITORING_ENABLE
  MonitoringDebug( sc->mon, "wrapper exited.\n");
#endif
